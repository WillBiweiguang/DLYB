    ...
    public class DefaultVirtualPathProvider : IVirtualPathProvider {
        ...
        public virtual string ToAppRelative(string virtualPath) {
            ...
            try {
                ...
                if (!result.StartsWith("~/")) {
                    Logger.Info("Path '{0}' cannot be made app relative: Path returned ('{1}') is not app relative.", virtualPath, result);
                    return null;
                }
                return result;
            }
                ...
                Logger.Error(e, "Path '{0}' cannot be made app relative", virtualPath);
                return null;
            }
        }
            ...
            if (virtualPath.IndexOf("..") >= 0) {
                ...
                if (!string.IsNullOrEmpty(rootPrefix)) {
                    ...
                    foreach (var term in terms) {
                        if (term == "..") {
                            if (depth == 0) {
                                Logger.Info("Path '{0}' cannot be made app relative: Too many '..'", virtualPath);
                                return true;
                            }
                            depth--;
                        }
        ...
        public virtual bool TryFileExists(string virtualPath) {
            ...
            try {
                ...
                Logger.Error( e,"File '{0}' can not be checked for existence. Assuming doesn't exist.", virtualPath);
                return false;
            }
        }
