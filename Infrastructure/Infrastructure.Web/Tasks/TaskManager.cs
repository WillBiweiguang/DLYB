...
namespace Infrastructure.Web.Tasks
{
    ...
    public partial class TaskManager
    {
        ...
        /// </summary>
        public void Initialize(IList<string> jobs)
        {
            _taskThreads.Clear();
            ...
            foreach (var scheduleTaskGrouped in scheduleTasks.GroupBy(x => new { StartDateTimeUtc = x.StartDateUtc, x.Seconds }))
            {
                //create a thread
                var taskThread = new TaskThread
                                     {
                                         Seconds = scheduleTaskGrouped.Key.Seconds,
                                         StartDateUtc = scheduleTaskGrouped.Key.StartDateTimeUtc
                                     };
                foreach (var scheduleTask in scheduleTaskGrouped)
                {
                    var task = new ProjectTask(scheduleTask);
            ...
            var notRunTasks = scheduleTasks
                ...
                //changed by john.xie
                .Where(x => x.Seconds >= NotRunTasksInterval && x.StartDateUtc == null)
