...
namespace Infrastructure.Web.MVC.Handlers
{
    ...
    public class StaticHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        //public static Func<HttpContext,bool> acAuth{set;get;}

        private static bool acAuth(HttpContext context)
        {
            return context.User.Identity.IsAuthenticated;
        }


        internal StaticHandler() {
        }
        ...
        private static FileInfo GetFileInfo(string virtualPathWithPathInfo, string physicalPath, HttpResponse response) {
            ...
            try {
                ...
                //    throw new HttpException(HttpStatus.NotFound, 
                //                            "Error_trying_to_enumerate_files");
                //else
                    throw new HttpException(HttpStatus.NotFound, 
                                            "Error_trying_to_enumerate_files", 
                                            ioEx);
            ...
            if ((((int)fileInfo.Attributes) & ((int)FileAttributes.Directory)) != 0) {
                if (StringEndsWith(virtualPathWithPathInfo, '/')) {
                    // Just return 403
                    throw new HttpException(HttpStatus.Forbidden,
                                            "Missing_star_mapping");
                }
                else {
                ...
                if (byteRangesCount >= byteRanges.Length) {
                    ...
                    unsafe {
                        fixed (ByteRange * src = byteRanges, dst = buffer) {
                            memcpyimpl((byte*)src, (byte*)dst, byteCount);
                        }
                    }
                    byteRanges = buffer;
                }
        ...
        internal static void ProcessRequestInternal(HttpContext context, string overrideVirtualPath) {
            ...
            if (StringStartsWithIgnoreCase(rangeHeader, "bytes")
        ...
        //    response.AppendHeader("Testaaa", "bytes");
        //    response.AppendHeader("Accept-Ranges", "bytes");
