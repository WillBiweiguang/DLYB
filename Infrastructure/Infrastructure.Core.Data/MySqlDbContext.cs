// -----------------------------------------------------------------------
//  <copyright file="CodeFirstDbContext.cs" company="DLYB">
//      Copyright (c) 2014 DLYB. All rights reserved.
//  </copyright>
//  <last-editor>@DLYB</last-editor>
//  <last-date>2015-04-22 17:34</last-date>
// -----------------------------------------------------------------------
...
namespace Infrastructure.Core.Data
{
    ...
    /// </summary>
    public class MySqlDbContext : DbContext, IUnitOfWork, IDependency
    {
        private static readonly ILogger Logger = LogManager.GetLogger(typeof(MySqlDbContext));

        /// <summary>
        /// 初始化一个<see cref="MySqlDbContext"/>类型的新实例
        /// </summary>
        ...
        /// <summary>
        /// 使用连接名称或连接字符串 初始化一个<see cref="MySqlDbContext"/>类型的新实例
        /// </summary>
        ...
        {
            string name = ConfigurationManager.AppSettings.Get("ConnectionString") ?? "DefaultConnection";
            return name;
        }
        ...
        internal int SaveChanges(bool validateOnSaveEnabled)
        {
            ...
            {
                if (e.InnerException != null && e.InnerException.InnerException is MySqlException)
                {
                    //SqlException sqlEx = e.InnerException.InnerException as SqlException;
                    MySql.Data.MySqlClient.MySqlException sqlEx = e.InnerException.InnerException as MySqlException;
                    string msg = DataHelper.GetSqlExceptionMessage(sqlEx.Number);
                    //deadlock
                    if (string.IsNullOrEmpty(msg) && sqlEx.Number == 1205)
                    {
                        Thread.Sleep(1000);
                        return base.SaveChanges();
                    }
                    throw new DLYBException("提交数据更新时发生异常：" + msg, sqlEx);
                }
                throw;
            }
            finally
        ...
        internal async Task<int> SaveChangesAsync(bool validateOnSaveEnabled)
        {
            ...
            catch (DbUpdateException e)
            {
                if (e.InnerException != null && e.InnerException.InnerException is MySqlException)
                {
                    MySqlException sqlEx = e.InnerException.InnerException as MySqlException;
                    string msg = DataHelper.GetSqlExceptionMessage(sqlEx.Number);
                    throw new DLYBException("提交数据更新时发生异常：" + msg, sqlEx);
                }
                throw;
