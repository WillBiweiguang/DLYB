...
namespace Infrastructure.Core.Infrastructure
{
    ...
    public class EngineContext
    {
        ...
        protected static IEngine CreateEngineInstance(WebCoreConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'Infrastructure.Web.Infrastructure.IEngine' and cannot be configured in /configuration/nop/engine[@engineType] for that purpose.");
        ...
        public static IEngine Initialize(bool forceRecreate)
        {
            ...
            {
                var config = ConfigurationManager.GetSection("WebCoreConfig") as WebCoreConfig;
