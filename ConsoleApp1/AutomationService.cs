...
namespace ConsoleApp1
{
    public class AutomationService
    {
        public static void Dotask()
        {
            ...
            {
                string sql = "select Id from t_BeamInfo b where Id > 100 and (AutoRegStatus is null or AutoRegStatus < 6) and not EXISTS (select 1 from t_weldcategorylabeling t where b.Id = t.BeamId and t.IsDeleted <> 1) and EXISTS(select 1 from t_ProjectInfo p where b.ProjectId = p.Id and p.IsDeleted = 0 and p.Status = '未完成') limit 0,1 ";
                string sqlUpdatestart = "update t_BeamInfo set AutoRegStatus = AutoRegStatus + 1 where Id = {0}";
        ...
        public static void DoAuto(string beamId)
        {
            ...
            try
            {
                selenium.Navigate().GoToUrl("http://111.207.36.126:4001/Account/backLogin?userName=auto-user");
