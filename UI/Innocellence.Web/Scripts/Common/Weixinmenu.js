...
DLYB.menu = {
    ...
    init: function () {
        ...
        DLYB.menu.token = $('#tokenStr').val();

        $("#buttonDetails_type").change(DLYB.menu.typeChanged);
            ...
            $.getJSON('/Menu/GetToken?t=' + Math.random(), { appId: $('#menuLogin_AppId').val(), appSecret: $('#menuLogin_AppSecret').val() },
                function (json) {
                    if (json.access_token) {
                        DLYB.menu.setToken(json.access_token);
                    } else {
        ...
        $('#menuLogin_SubmitOldToken').click(function () {
            DLYB.menu.setToken($('#menuLogin_OldToken').val());
        });

        $('#btnGetMenu').click(function () {
            menuState.html('获取菜单中...');
            $.getJSON('./GetMenu?t=' + Math.random(), { token: DLYB.menu.token }, function (json) {
                if (json.menu) {
        ...
        $('#btnDeleteMenu').click(function () {
            ...
            $.getJSON('./DeleteMenu?t=' + Math.random(), { token: DLYB.menu.token }, function (json) {
                if (json.Success) {
