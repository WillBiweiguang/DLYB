...
namespace DLYB.Web
{
    ...
    {
        public static void Dotask(string beamId)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connStr))
            {
                ...
                string sql = string.Format("select Id from t_BeamInfo b where Id = {0} and (AutoRegStatus is null or AutoRegStatus < 6) and not EXISTS (select 1 from t_weldcategorylabeling t where b.Id = t.BeamId and t.IsDeleted <> 1) and EXISTS(select 1 from t_ProjectInfo p where b.ProjectId = p.Id and p.IsDeleted = 0 and p.Status = '未完成') limit 0,1 ", beamId);
                string sqlUpdatestart = "update t_BeamInfo set AutoRegStatus = 1 where Id = {0}";
                string sqlUpdateend = "update t_BeamInfo set AutoRegStatus = AutoRegStatus + 1 where Id = {0}";
                conn.Open();
                string status = "";
                using (MySqlCommand cmd = new MySqlCommand(sql, conn))
                {
                    ...
                    using (MySqlDataReader dataReader = cmd.ExecuteReader())
                    {
                        ...
                        {
                            status = dataReader[0].ToString();
                        }
                    }
                }
                var sql2 = string.Format(sqlUpdatestart, beamId);
                if (string.IsNullOrEmpty(beamId) || string.IsNullOrEmpty(status))
                {
                    return;
                }
        ...
        public static void DoAuto(string beamId)
        {
            InternetExplorerOptions options = new InternetExplorerOptions();
            options.IntroduceInstabilityByIgnoringProtectedModeSettings = true;
            //IWebDriver driver = new InternetExplorerDriver(Path.Combine(GetBasePath, @"Drivers\\"), options);
            IWebDriver selenium = new InternetExplorerDriver(options);
            try
            {
                selenium.Navigate().GoToUrl("http://111.207.36.126:4001/Account/Login");
                //selenium.Navigate().GoToUrl("http://42.202.130.245:3001/Account/backLogin?userName=auto-user");
