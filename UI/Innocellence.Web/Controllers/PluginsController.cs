...
namespace DLYB.Web.Controllers
{
    public class PluginsController : BaseController<PluginModel, PluginView>
    {
        ...
        public ActionResult ZipFile(string systemName)
        {
            ...
            var strPath = string.Format("{0}\\Plugins\\{1}", Server.MapPath("~/"), pluginDescriptor.SystemName);

            var strPathTemp = string.Format("{0}\\{1}.zip", Server.MapPath("~/pluginsTemp"), Guid.NewGuid());
