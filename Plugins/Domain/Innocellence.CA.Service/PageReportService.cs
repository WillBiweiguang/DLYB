...
namespace DLYB.CA.Services
{
    ...
    public partial class PageReportService : BaseService<PageReport>, IPageReportService
    {
              ...
              PageCondition con)
        {
            ...
            var q = from p in Repository.Entities.Where(predicate) group p by p.AccessDate into g select new PageReportView  { AccessDate= g.Key.ToString(), TotalVisitorCount = g.Sum(p => p.VisitorCount), TotalVisitTimes = g.Sum(p => p.VisitTimes) };
