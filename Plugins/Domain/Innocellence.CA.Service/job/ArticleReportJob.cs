...
namespace DLYB.CA.Service.job
{
    public class ArticleReportJob : ICustomerJob
    {
        ...
        private readonly string _jobName = JobName.ArticleReportJob.ToString();//"ArticleReportJob";
        ...
        private IList<ArticleReport> GetNewsListByIds(IList<UserBehavior> userBehaviors)
        {
            ...
            var list = _articleInfoService.Repository.Entities.Where(x => articleIds.Contains(x.Id)).Select(x => new { x.ArticleTitle, x.AppId, x.Id, x.ArticleCateSub }).ToList();
            ...
            var outs = logs.Select(x =>
            {
                ...
                var category = CommonService.lstCategory.FirstOrDefault(z => z.CategoryCode == news.ArticleCateSub);
