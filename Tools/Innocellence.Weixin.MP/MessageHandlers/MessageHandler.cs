...
using DLYB.Weixin.MP.AppStore;
using DLYB.Weixin.MP.Entities;
using DLYB.Weixin.MP.Entities.Request;
...
namespace DLYB.Weixin.MP.MessageHandlers
{
    /// <summary>
    /// 微信请求的集中处理方法
    /// 此方法中所有过程，都基于DLYB.Weixin.MP的基础功能，只为简化代码而设。
    /// </summary>
        ...
        where TC : class, IMessageContext<IRequestMessageBase, IResponseMessageBase>, new()
    {
        ...
        /// 这里是DLYB.Weixin.MP，根据请求消息半段在ResponseDocument基础上是否需要再次进行加密（每次获取重新加密，所以结果会不同）
        /// </summary>
        ...
        /// <param name="maxRecordCount">单个用户上下文消息列表储存的最大长度</param>
        /// <param name="developerInfo">微微嗨开发者信息，如果不为空，则优先请求云端应用商店的资源</param>
        public MessageHandler(XDocument requestDocument, PostModel postModel = null, int maxRecordCount = 0, DeveloperInfo developerInfo = null, bool isDebug = false)
        ...
        public virtual void AfterGetData(string AppID, PostModel postModel)
        {
