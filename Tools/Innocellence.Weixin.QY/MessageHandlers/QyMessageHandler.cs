...
namespace DLYB.Weixin.QY.MessageHandlers
{
    ...
    {
        public delegate void QyMessageEventHandler(IQyMessageHandler messageHandler);
        public static event QyMessageEventHandler QyMessageHandlerEvent;


        //public Action<EncryptPostData, PostModel> actAfterGetData;

        //public Action<string, PostModel> actAfterDecryptData;
        ...
        /// 这里是DLYB.Weixin.QY，应当在ResponseDocument基础上进行加密（每次获取重新加密，所以结果会不同）
        /// </summary>
        ...
        public QyMessageHandler(Stream inputStream, PostModel postModel, int maxRecordCount = 0,bool isDebug=false)
            : base(inputStream, maxRecordCount, postModel, isDebug)
        {
           // _isDebug = isDebug;
        }

        public QyMessageHandler(XDocument requestDocument, PostModel postModel, int maxRecordCount = 0, bool isDebug = false)
            : base(requestDocument, maxRecordCount, postModel, isDebug)
        {
            //_isDebug = isDebug;
        }
        ...
        public QyMessageHandler(RequestMessageBase requestMessageBase, int maxRecordCount = 0, object postData = null, bool isDebug = false)
            : base(requestMessageBase, maxRecordCount, postData, isDebug)
        {
            RequestMessage = requestMessageBase;
        }
        ...
        public virtual void AfterDecryptData(string strXML,PostModel postModel)
        {
        ...
        public override void Execute()
        {
            ...
            try
            {
                ...
                // 先处理插件注册的事件
                if (QyMessageHandlerEvent != null)
                {
                    QyMessageHandlerEvent(this);
                }

                // 如果response里有东西了，说明已经处理过了。不再处理。
