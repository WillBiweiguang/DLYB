...
namespace DLYB.Weixin.QY.AdvancedAPIs
{
    public static class MailListApi
    {
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static WxJsonResult UpdateDepartment(string accessToken, DepartmentList dept, int timeOut = Config.TIME_OUT)
        {
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static DelTagMemberResult DelTagMember(string accessToken, int tagId, string[] userList,int[] partyList=null, int timeOut = Config.TIME_OUT)
        {
        ...
        /// <param name="accessToken"></param>
        /// <returns></returns>
        public static BatchUserResult BatchUpdateUser(string accessToken, BatchUser data, int timeOut = Config.TIME_OUT)
        {
            //  BatchUser data = GetUserObject(strMediaID, "");

            var url = string.Format("https://qyapi.weixin.qq.com/cgi-bin/batch/syncuser?access_token={0}", accessToken);

            return CommonJsonSend.Send<BatchUserResult>(accessToken, url, data, CommonJsonSendType.POST, timeOut);
        }
        ...
        /// <param name="accessToken"></param>
        /// <returns></returns>
        public static BatchUserResult BatchReplaceUser(string accessToken, BatchUser data, int timeOut = Config.TIME_OUT)
        {
            // BatchUser data = GetUserObject(strMediaID, "");
        ...
        /// <param name="accessToken"></param>
        /// <returns></returns>
        public static BatchUserResult BatchReplaceDept(string accessToken, BatchUser data, int timeOut = Config.TIME_OUT)
        {
            //BatchUser data = GetUserObject(strMediaID, "");
        ...
        /// <param name="accessToken"></param>
        /// <returns></returns>
        public static BatchUserResult BatchInvite(string accessToken, BatchUser data, int timeOut = Config.TIME_OUT)
        {
            //  BatchUser data = GetUserObject(strMediaID,"");
        ...
        /// <returns></returns>
        public static JobResult<T> BatchJobStatus<T>(string accessToken, string strJobID)
        {

            var url = string.Format("https://qyapi.weixin.qq.com/cgi-bin/batch/getresult?access_token={0}&jobid={1}", accessToken, strJobID);

            return Get.GetJson<JobResult<T>>(url);
        }


        private static BatchUser GetUserObject(string mediaID, string strAESKey, string strType)
        {
            BatchUser data = new BatchUser()
            {
                media_id = mediaID,
                callback = new callback() { encodingaeskey = strAESKey }
            };
