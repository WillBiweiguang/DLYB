...
using DLYB.Weixin.QY.AdvancedAPIs.Media;
...
namespace DLYB.Weixin.QY.AdvancedAPIs
{
    ...
    public static class MediaApi
    {
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static UploadTemporaryResultJson Upload(string accessToken, UploadMediaFileType type, Dictionary<string, Stream> fileDictionary, string media, int timeOut = Config.TIME_OUT)
        {
            ...
            dataDictionary["media"] = media;
            return HttpUtility.Post.PostFileGetJson<UploadTemporaryResultJson>(url, null, fileDictionary, dataDictionary, null, timeOut);
        }
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static UploadForeverResultJson UploadPermanent(string accessToken, string agentid, UploadMediaFileType type, Dictionary<string, Stream> fileDictionary, string media, int timeOut = Config.TIME_OUT)
        {
            var url = string.Format("https://qyapi.weixin.qq.com/cgi-bin/material/add_material?agentid={0}&type={1}&access_token={2}", agentid, type.ToString(), accessToken);
            var dataDictionary = new Dictionary<string, string>();
            dataDictionary["media"] = media;
            return HttpUtility.Post.PostFileGetJson<UploadForeverResultJson>(url, null, fileDictionary, dataDictionary, null, timeOut);
        }
