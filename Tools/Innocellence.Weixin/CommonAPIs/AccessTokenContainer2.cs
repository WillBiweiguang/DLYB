...
namespace Innocellence.Weixin.CommonAPIs
{
    ...
    public class AccessTokenContainer
    {
        ...
        {
            AccessTokenCollection[appId+"_"+appSecret] = new AccessTokenBag()
            {
                AppId = appId,
                AppSecret = appSecret,
        ...
        public static string TryGetToken(string appId, string appSecret, Func<string, string, string, AccessTokenResult> func, bool getNewToken = false)
        {
            lock (Lock)
            {
                if (!CheckRegistered(appId,appSecret) || getNewToken)
                {
        ...
        {
            if (!AccessTokenCollection.ContainsKey(appId+"_"+appSecret))
            {

                throw new WeixinException("此appId尚未注册，请先使用AccessTokenContainer.Register完成注册（全局执行一次即可）！");
            }

            var accessTokenBag = AccessTokenCollection[appId + "_" + appSecret];
            lock (accessTokenBag.Lock)
            {
                if (getNewToken || accessTokenBag.ExpireTime <= DateTime.Now)
                {
                    //已过期，重新获取
                    accessTokenBag.AccessTokenResult = func(accessTokenBag.AppId, accessTokenBag.AppSecret,"");// CommonApi.GetToken(accessTokenBag.AppId, accessTokenBag.AppSecret);
        ...
        /// <param name="appId"></param>
        /// <returns></returns>
        public static bool CheckRegistered(string appId, string appSecret)
        {
            return AccessTokenCollection.ContainsKey(appId+"_"+appSecret);
        }
    }
}
