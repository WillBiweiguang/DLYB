...
namespace Innocellence.Weixin.CommonAPIs
{
    public static class AccessTokenHandlerWapper
    {
        ...
        /// <returns></returns>
        public static T Do<T>(string appId, string appSecret, Func<string, T> fun, Func<string, string, AccessTokenResult> funcToken, bool retryIfFaild = true) where T : WxJsonResult
        {
            T result = null;
            try
            {
                var accessToken = AccessTokenContainer.TryGetToken(appId, appSecret,funcToken, false);
            ...
            catch (ErrorJsonResultException ex)
            {
                ...
                {
                    //尝试重新验证
                    var accessToken = AccessTokenContainer.TryGetToken(appId, appSecret,funcToken, true);
                    result = Do<T>(appId, appSecret, fun,funcToken, false);
                }
            }
            return result;
        }
