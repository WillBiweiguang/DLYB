...
namespace DLYB.Weixin.Containers
{
    ...
    /// IBaseContainer
    /// </summary>
    public interface IBaseCacheContainer
    {
    }
    ...
    /// <summary>
    /// 带AccessTokenBag泛型的IBaseContainer
    /// </summary>
    /// <typeparam name="TBag"></typeparam>
    public interface IBaseCacheContainer<TBag> : IBaseCacheContainer where TBag : IBaseContainerBag, new()
    {
    }
    ...
    /// <typeparam name="TBag"></typeparam>
    [Serializable]
    public abstract class BaseContainer<TBag> : IBaseCacheContainer<TBag> where TBag : class, IBaseContainerBag, new()
    {
        /// <summary>
        ...
        private static IContainerCacheStragegy /*IBaseCacheStrategy<string,Dictionary<string, TBag>>*/ Cache
        {
            ...
            {
                //使用工厂模式或者配置进行动态加载
                return CacheStrategyFactory<TBag>.GetContainerCacheStragegyInstance();
            }
        }
        ...
        //            //保存到缓存列队，等待执行
        //            DLYBMessageQueue mq = new DLYBMessageQueue();
        //            var mqKey = DLYBMessageQueue.GenerateKey("ContainerItemCollection", typeof(BaseContainer<TBag>), cacheKey, "InsertItemCollection");
        //            mq.Add(mqKey, () =>
        //            {
        //                var containerCacheStragegy = CacheStrategyFactory.GetContainerCacheStragegyInstance();
        //                containerCacheStragegy.InsertToCache(cacheKey, itemCollection);//插入到缓存
        ...
        {
            if (Cache.CheckExisted(key))
            {
                return Cache.Get(key) as TBag;
            }

            return default(TBag);
        }

        public static TBag GetItem(string key)
        {
        ...
        public static TK TryGetItem<TK>(string key, Func<TBag, TK> property)
        {
            if (Cache.CheckExisted(key))
            {
                var item = Cache.Get(key) as TBag;
        ...
        public static void Update(string key, Action<TBag> partialUpdate)
        {
            ...
            {
                Cache.RemoveFromCache(key);//移除对象
            }
            else
            {
                TBag newBag;
                ...
                {
                    newBag = Cache.Get(key) as TBag;
                }

                partialUpdate(newBag);//更新对象
                var cacheKey = key;// GetCacheKey();
                Cache.Update(cacheKey, newBag);//更新到缓存，TODO：有的缓存框架可一直更新Hash中的某个键值对
            }
        }
