...
namespace DLYB.Weixin.HttpUtility
{
    public static class RequestUtility
    {
        ...
        /// <returns></returns>
        public static string HttpPost(string url, CookieContainer cookieContainer = null, Stream postStream = null, Dictionary<string, Stream> fileDictionary = null, string refererUrl = null, Encoding encoding = null, int timeOut = Config.TIME_OUT)
        {

           // Log4NetLog.Logger.DebugFormat("HttpPost :{0}", url);
            ...
            if (formUploadFile)
            {
                ...
                foreach (var file in fileDictionary)
                {
                    try
                    {
                        ...
                        //准备文件流
                        using (var fileStream = (file.Value == null ? FileHelper.GetFileStream(fileName) : file.Value))
                        {
                            string formdata = null;
                            if (fileStream != null)
                            {
                                //存在文件
                                formdata = string.Format(fileFormdataTemplate, Path.GetFileName(fileName), fileName /*Path.GetFileName(fileName)*/);
                            }
                            else
                            {
                                //不存在文件或只是注释
                                formdata = string.Format(dataFormdataTemplate, Path.GetFileName(fileName), fileName);
                            }
        ...
        /// <returns></returns>
        [Obsolete("请使用DLYB.Weixin.BrowserUtility.BroswerUtility.SideInWeixinBroswer()方法")]
