/*----------------------------------------------------------------
    ...
    文件名：DLYBMessageQueue.cs
    文件功能描述：DLYBMessageQueue消息列队
...
namespace DLYB.Weixin.MessageQueue
{
    ...
    /// 消息列队
    /// </summary>
    public class DLYBMessageQueue
    {
        /// <summary>
        /// 列队数据集合
        /// </summary>
        private static Dictionary<string, DLYBMessageQueueItem> MessageQueueDictionary = new Dictionary<string, DLYBMessageQueueItem>(StringComparer.OrdinalIgnoreCase);
        ...
        /// <summary>
        /// 获取DLYBMessageQueueItem
        /// </summary>
        /// <param name="key"></param>
        /// <returns></returns>
        public DLYBMessageQueueItem GetItem(string key)
        {
            lock (MessageQueueSyncLock)
            {
        ...
        public DLYBMessageQueueItem Add(string key, Action action)
        {
            lock (MessageQueueSyncLock)
            {
                //if (!MessageQueueDictionary.ContainsKey(key))
                ...
                var mqItem = new DLYBMessageQueueItem(key, action);
